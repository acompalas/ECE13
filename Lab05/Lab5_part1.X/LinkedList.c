...
ListItem *LinkedListNew(char *data) {
    ListItem *result = (struct ListItem*) malloc(sizeof (ListItem));
    if (result == NULL) {
        return NULL;
    } else {
        result->data = data;
...
ListItem *LinkedListCreateAfter(ListItem *item, char *data) {
    ListItem *new_item = LinkedListNew(data);
    if (item->nextItem == NULL) {
        item->nextItem = new_item;
        new_item->previousItem = item;
        new_item->nextItem = NULL;
        return new_item;
    } else if (item == NULL) {
        return new_item;
    } else {
        item->nextItem->previousItem = new_item;
        new_item->nextItem = item->nextItem;
        item->nextItem = new_item;
        new_item->previousItem = item;
        return new_item;
    }
...
ListItem *LinkedListGetFirst(ListItem *list) {
    ...
    if (list->previousItem == NULL && list->nextItem == NULL) {
        return list;
    }
...
int LinkedListSwapData(ListItem *firstItem, ListItem *secondItem) {
    char *a = firstItem->data;
    char *b = secondItem->data;
